...
select
      ...
      (
       select date,
          ...
          from TABLE_DATE_RANGE([74773001.ga_sessions_], DATE_ADD(CURRENT_TIMESTAMP(), -15, 'DAY'), DATE_ADD(CURRENT_TIMESTAMP(), -2, 'DAY'))
          ,TABLE_DATE_RANGE([74762698.ga_sessions_], DATE_ADD(CURRENT_TIMESTAMP(), -15, 'DAY'), DATE_ADD(CURRENT_TIMESTAMP(), -2, 'DAY')), TABLE_DATE_RANGE([108855442.ga_sessions_], DATE_ADD(CURRENT_TIMESTAMP(), -15, 'DAY'), DATE_ADD(CURRENT_TIMESTAMP(), -2, 'DAY'))
     ...
     join each
       ...
       select date,
           ...
           where  hits.customDimensions.index=1 and hits.customDimensions.value in ('BD','BG','BN','HK','IN','KZ','MY','PH','RO','SG','TH','TW','SA','EG','GE')
