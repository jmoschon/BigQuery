...
(select date(vendor_visits.date) date,vendor_visits.vendor_id, countries.country,vendor_visits.device.operatingSystem, vendor_visits.Users, vendor_visits.channel, sum(vendor_visit) visits from
            ...
            flatten (
                      (
                        ...
                        from TABLE_DATE_RANGE([74773001.ga_sessions_], DATE_ADD(CURRENT_TIMESTAMP(), -3, 'DAY'), DATE_ADD(CURRENT_TIMESTAMP(), -1, 'DAY'))
                         where REGEXP_MATCH(hits.eventInfo.eventAction,'(Restaurant Detail Page)')
        ...
        from
            flatten (
                      (
                        ...
                        from TABLE_DATE_RANGE([74773001.ga_sessions_], DATE_ADD(CURRENT_TIMESTAMP(), -3, 'DAY'), DATE_ADD(CURRENT_TIMESTAMP(), -1, 'DAY'))
                         where REGEXP_MATCH(hits.eventInfo.eventAction,'(Checkout)')
                        group each by 1,2,4,5,6,7,8,9,10
                       ), visitid
                     ) a
        ) vendor_visits
      ...
      (
        ...
        on a.vendor_id = c.vendor_id and a.date = c.date and a.countries.country = c.countries.country and a.vendor_visits.device.operatingsystem=c.vendor_visits.device.operatingsystem and c.vendor_visits.users=b.vendor_visits.users and a.vendor_visits.channel=c.vendor_visits.channel