    ...
    --        dim_area.area as area,
            ...
            sum(CR4b_completion) as CR4b_completion,
    ...
    case when sum(case when hits.eventinfo.eventaction='CR4e Completion' then 1 else 0 end)>=1 then 1 else 0 end as CR4e_completion
        ...
        from TABLE_DATE_RANGE([86553600.ga_sessions_], DATE_ADD(CURRENT_TIMESTAMP(), -20, 'DAY'), DATE_ADD(CURRENT_TIMESTAMP(), -2, 'DAY'))
        --, TABLE_DATE_RANGE([87584217.ga_sessions_], DATE_ADD(CURRENT_TIMESTAMP(), -20, 'DAY'), DATE_ADD(CURRENT_TIMESTAMP(), -2, 'DAY')), TABLE_DATE_RANGE([108857524.ga_sessions_], DATE_ADD(CURRENT_TIMESTAMP(), -20, 'DAY'), DATE_ADD(CURRENT_TIMESTAMP(), -2, 'DAY'))
