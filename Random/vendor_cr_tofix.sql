...
(select country, city_id, area_id, OperatingSystem,user_type, vendor_id,'' as vendor_code, date, 'APP' as platform ,medium, ''as landingpage
...
select a.fullvisitorid , a.visitid, country, city, area, city_id, area_id, OperatingSystem, v.vendor_id as vendor_id, a.date as date, medium,user_type,
...
case when sum(case when (REGEXP_MATCH(g.hits.appInfo.ScreenName,'^(Checkout Screen|Delivery Method Screen|Email Verification Screen|Address Book Screen|User Details Screen)')) then 1 else 0 end)>=1 then 1 else 0 end as CR4a_completion,
...
select a.*, v.vendor_id,g.hits.appInfo.ScreenName,g.hits.eventinfo.eventaction,g.device.operatingSystem, c.country as country, cities.city as city, areas.area as area, area_id.city_id as city_id, area_id.area_id as area_id, c.device.operatingSystem as OperatingSystem, c.medium as medium , user_type
  from
    (
  ...
  from TABLE_DATE_RANGE([74773001.ga_sessions_], DATE_ADD(CURRENT_TIMESTAMP(), -2, 'DAY'), DATE_ADD(CURRENT_TIMESTAMP(), -2, 'DAY')),TABLE_DATE_RANGE([74762698.ga_sessions_], DATE_ADD(CURRENT_TIMESTAMP(), -2, 'DAY'), DATE_ADD(CURRENT_TIMESTAMP(), -2, 'DAY')), TABLE_DATE_RANGE([108855442.ga_sessions_], DATE_ADD(CURRENT_TIMESTAMP(), -2, 'DAY'), DATE_ADD(CURRENT_TIMESTAMP(), -2, 'DAY'))
      ...
      and (REGEXP_MATCH(hits.appInfo.ScreenName,'^(Checkout Screen|Delivery Method Screen|Email Verification Screen|Address Book Screen|User Details Screen|Restaurant[^s])|Checkout Confirmation Screen|Variations and Toppings Screen|Restaurant Products Screen|Restaurant Categories Screen|Restaurant Menus Screen')
...
left outer join each /*assigning heats and screen names */
  ...
  from TABLE_DATE_RANGE([74773001.ga_sessions_], DATE_ADD(CURRENT_TIMESTAMP(), -2, 'DAY'), DATE_ADD(CURRENT_TIMESTAMP(), -2, 'DAY')),TABLE_DATE_RANGE([74762698.ga_sessions_], DATE_ADD(CURRENT_TIMESTAMP(), -2, 'DAY'), DATE_ADD(CURRENT_TIMESTAMP(), -2, 'DAY')), TABLE_DATE_RANGE([108855442.ga_sessions_], DATE_ADD(CURRENT_TIMESTAMP(), -2, 'DAY'), DATE_ADD(CURRENT_TIMESTAMP(), -2, 'DAY'))
  where device.operatingSystem in('Android','iOS')
  and (REGEXP_MATCH(hits.appInfo.ScreenName,'^(Checkout Screen|Delivery Method Screen|Email Verification Screen|Address Book Screen|User Details Screen|Restaurant[^s])|Checkout Confirmation Screen|Variations and Toppings Screen|Checkout _ Payment option')
  or hits.eventinfo.eventaction in('RDP _ Add Item to cart','RDP - Minimum value reached (On Basket)', 'Checkout _ Payment option'))
...
inner join each /*country */
  ...
  select *
  from
  (select visitid, fullVisitorId, upper(hits.customDimensions.value) as country, device.operatingSystem
  , case when trafficSource.medium='(none)' then 'direct'
            when trafficSource.medium='push' then 'push'
            else 'other'
      end as medium
  from TABLE_DATE_RANGE([74773001.ga_sessions_], DATE_ADD(CURRENT_TIMESTAMP(), -2, 'DAY'), DATE_ADD(CURRENT_TIMESTAMP(), -2, 'DAY')),TABLE_DATE_RANGE([74762698.ga_sessions_], DATE_ADD(CURRENT_TIMESTAMP(), -2, 'DAY'), DATE_ADD(CURRENT_TIMESTAMP(), -2, 'DAY')), TABLE_DATE_RANGE([108855442.ga_sessions_], DATE_ADD(CURRENT_TIMESTAMP(), -2, 'DAY'), DATE_ADD(CURRENT_TIMESTAMP(), -2, 'DAY'))
  where device.operatingSystem in('Android','iOS') and hits.customDimensions.index=1
...
on cities.fullvisitorid=a.fullvisitorid and cities.visitid=a.visitid

left join each

(select a.* from
flatten (
(select fullVisitorId, unique (visitId) as visitid, hits.customDimensions.value as area
from TABLE_DATE_RANGE([74773001.ga_sessions_], DATE_ADD(CURRENT_TIMESTAMP(), -2, 'DAY'), DATE_ADD(CURRENT_TIMESTAMP(), -2, 'DAY')), TABLE_DATE_RANGE([74762698.ga_sessions_], DATE_ADD(CURRENT_TIMESTAMP(), -2, 'DAY'), DATE_ADD(CURRENT_TIMESTAMP(), -2, 'DAY')), TABLE_DATE_RANGE([108855442.ga_sessions_], DATE_ADD(CURRENT_TIMESTAMP(), -2, 'DAY'), DATE_ADD(CURRENT_TIMESTAMP(), -2, 'DAY'))
where hits.customDimensions.index=4
group each by 1,3), visitid) a) areas

on areas.fullvisitorid=a.fullvisitorid and areas.visitid=a.visitid
...
,(
select  country,city_id,area_id,'' as OperatingSystem,user_type, '' as vendor_id ,vendor_code, date, 'WEB' as platform, medium, landingpage
,sum(CR3_start) as CR3_start
,sum(CR3a_completion) as CR3a_completion
,sum(CR3b_completion) as CR3b_completion
...
select a.fullVisitorId as fullVisitorId
        ...
        , cities.city as city
        , areas.area as area
        , area_id.city_id as city_id
        , area_id.area_id as area_id
        ,medium
        ,landingPage
        , user_type
...
on area_id.fullvisitorid=a.fullvisitorid and area_id.visitid=a.visitid
    ...
    left outer join each /*CR steps*/
       ...
       left outer join each /*CR steps_2*/
          ...
          from TABLE_DATE_RANGE([86553600.ga_sessions_], DATE_ADD(CURRENT_TIMESTAMP(), -2, 'DAY'), DATE_ADD(CURRENT_TIMESTAMP(), -2, 'DAY')),TABLE_DATE_RANGE([87584217.ga_sessions_], DATE_ADD(CURRENT_TIMESTAMP(), -2, 'DAY'), DATE_ADD(CURRENT_TIMESTAMP(), -2, 'DAY'))
