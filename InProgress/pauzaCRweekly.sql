...
(Select date(b.date) as ga_date,
        ...
        (sum(cr4_compl.CR4_completion)) as co_so_complete
  from
     ...
     (        select date,
              ...
              case when sum(case when(regexp_match(hits.page.pagePath,'(zagreb|velika-gorica|order=)')) then 1 else 0 end)>=1 then 1 else 0 end as CR1_completion,
              case when sum(case when(regexp_match(hits.page.pagePath,'(zagreb|velika-gorica|order=)')) then 1 else 0 end)>=1 then 1 else 0 end as CR2_start,
              case when sum(case when(regexp_match(hits.page.pagePath,'/jelovnik/')) then 1 else 0 end)>=1 then 1 else 0 end as CR3_start,
              case when sum(case when(regexp_match(hits.page.pagePath,'(/order-preview/|order-process)')) then 1 else 0 end)>=1 then 1 else 0 end as CR4_start,
--              case when sum(case when(regexp_match(hits.page.pagePath,'/order-process')) then 1 else 0 end)>=1 then 1 else 0 end as CR4_complete
       from TABLE_DATE_RANGE([95921379.ga_sessions_], DATE_ADD(CURRENT_TIMESTAMP(), -80, 'DAY'), DATE_ADD(CURRENT_TIMESTAMP(), -2, 'DAY'))
    ...
    select c.date as date,
      ...
      from
            ...
            (
              ...
              where regexp_match(hits.page.pagePath,'/jelovnik/')
              group each by 1,2,3
            ) c
...
join each
            ...
            (
              Select  date,
               ...
               where regexp_match(hits.page.pagePath,'(zagreb|velika-gorica)')
               group each  by 1,2,3
             ) b
...
(
      ...
      from
              ...
              (
                ...
                where regexp_match(hits.page.pagePath,'(/order-preview/|order-process)')
                group each by 1,2,3
              ) c
...
(
      ...
      from
              ...
              (
                ...
                where regexp_match(hits.page.pagePath,'/orders/')
                group each by 1,2,3
              ) c
